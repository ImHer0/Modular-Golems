# Scale Up - Iron Nugget
execute at @s run kill @e[type=item,nbt={Item:{id:"minecraft:iron_nugget",count:1}},sort=nearest,limit=1]

# Increase scale level by 1
scoreboard players add @s mg_scale 1
...
execute at @s run particle minecraft:firework ~ ~1 ~ 0.3 1 0.3 0.05 20 normal
execute at @s run playsound minecraft:entity.player.levelup master @a ~ ~ ~ 1 0.5

# Display message
execute if score @s mg_scale matches 1 run tellraw @a[distance=..5] [{"text":"📏 Golem scaled to ","color":"aqua"},{"text":"0.5x","color":"gold","bold":true}]
...
execute if score @s mg_scale matches 4 run tellraw @a[distance=..5] [{"text":"📏 Golem scaled to ","color":"aqua"},{"text":"2.0x (MAX!)","color":"gold","bold":true}]
