# Netherite Hoe Farming Upgrade
# Check if already has this tier (refund if duplicate)
execute if entity @s[tag=mgfarming_netherite] run summon item ~ ~1 ~ {Item:{id:"minecraft:netherite_hoe",count:1}}
execute if entity @s[tag=mgfarming_netherite] run return fail
...
tag @s remove mgfarming_diamond

# Add new tier tags
tag @s add mgfarming_netherite
tag @s add mgfarming

# Kill the dropped hoe
kill @e[type=item,nbt={Item:{id:"minecraft:netherite_hoe"}},distance=..2,sort=nearest,limit=1]
...
# Message
tellraw @a[distance=..10] [{"text":"[Modular Golems] ","color":"gold"},{"text":"Golem upgraded to ","color":"gray"},{"text":"Netherite Farming","color":"green"},{"text":" (1/100 crop growth chance)","color":"dark_gray"}]
