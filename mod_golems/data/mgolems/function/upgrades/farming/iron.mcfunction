...
execute if entity @s[tag=mgfarming_iron] run return fail
...
# Add new tier tags
tag @s add mgfarming_iron
tag @s add mgfarming

# Kill the dropped hoe
kill @e[type=item,nbt={Item:{id:"minecraft:iron_hoe"}},distance=..2,sort=nearest,limit=1]
...
# Message
tellraw @a[distance=..10] [{"text":"[Modular Golems] ","color":"gold"},{"text":"Golem upgraded to ","color":"gray"},{"text":"Iron Farming","color":"green"},{"text":" (1/10000 crop growth chance)","color":"dark_gray"}]
